#!/usr/bin/env bash
# seed-pete-jamee.sh — Generate nuggets + companions for Pete Rango and Jamee Cornelia only.
...
# Only Pete Rango and Jamee Cornelia
declare -a SLUGS=("peterango" "jameecornelia")
declare -a ARTISTS=("Pete Rango" "Jamee Cornelia")
declare -a TITLES=("Oms at Play" "SLACK")
declare -a ALBUMS=("Savage Planet" "HARVEST")
declare -a TRACK_IDS=(
  "real::Pete Rango::Oms at Play::Savage Planet::spotify:track:7mYphBaMfblb6iu1saj3MC"
  "real::Jamee Cornelia::SLACK::HARVEST::spotify:track:5bU8cB57AfhTtO0qj9zy3X"
)
...
ls -1 "$DATA_DIR"/{peterango,jameecornelia}-*.json 2>/dev/null | sort
echo ""
echo "Nugget counts:"
for f in "$DATA_DIR"/{peterango,jameecornelia}-*-listen*.json; do
  [ -f "$f" ] || continue
  count=$(jq '.nuggets | length' "$f" 2>/dev/null || echo "?")
  echo "  $(basename "$f"): $count nuggets"
done
